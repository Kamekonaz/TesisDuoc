...
function Test({passFinalImage, imagen, setIsEditingImage}) {
    ...
    const toDataURL = () => {
        ...
        ctx.drawImage(image, 
            (image.naturalWidth/uwu["scale"] - canvas.width) /2 - uwu["to_move"]["X"]/uwu["scale"] - image.offsetLeft/2, 
            (image.naturalHeight/uwu["scale"] - canvas.height) /2 - uwu["to_move"]["Y"]/uwu["scale"] - image.offsetTop/2
        );
    ...
    return (
        <div className={`Test flex relative m-auto z-40 bg-gray-200 text-black`}>
            <div className="border border-black border-4 flex flex-col p-24">
                ...
                <div className="flex justify-between w-full mt-4">
                    <button className="select-none bg-red-500 text-white font-bold hover:bg-red-600 px-2 py-1 rounded-lg" onClick={()=>setIsEditingImage(false)}>Cancelar</button>
                    <button className="select-none bg-green-500 text-white font-bold hover:bg-green-600 px-2 py-1 rounded-lg" onClick={()=>saveIMG()}>Aceptar</button>
                </div>
                <img src={srctest} alt="" />
            </div>
